...
const deleteSpaceWork = async (req, res) => {
  try {
      ...
      for (let list of lists) {
        ...
        for (let task of tasks) {
          ...
          if (!resTask)
            return res
              .status(400)
              .send(
                "An error ocurred removing tasks of list. Please try again later"
              );
        }
        ...
        if (!resList)
          return res
            .status(400)
            .send(
              "An error ocurred removing lists of board. Please try again later"
            );
      }
    ...
    if (!result)
      return res.status(400).send("An error ocurred. Please try again later");
...
const addUsers = async (req, res) => {
  try {
    if (!req.body.email || !req.body.workspace_id)
      return res.status(400).send("incomplete data");
    ...
    if (!validUser.dbStatus)
      return res.status(400).send("The user is disabled");
    ...
    const arrUsers = Object.keys(space.user_id).map(
      (key) => space.user_id[key]
    );
...
const removeUser = async (req, res) => {
  try {
    ...
    if (validUser._id == req.user._id)
      return res.status(400).send("Your user is not have sufficent perms");
